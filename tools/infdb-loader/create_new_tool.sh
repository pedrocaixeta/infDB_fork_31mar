...
# Replace all occurrences of "infdb-loader" with the new name
echo "Replacing placeholders with $NEW_NAME..."
find "$TARGET_DIR" -type f -exec sed -i.bak "s/infdb-loader/$NEW_NAME/g" {} +
find "$TARGET_DIR" -type f -exec sed -i.bak "s/infdb_loader/$SNAKE_CASE_NAME/g" {} +
find "$TARGET_DIR" -type f -name "*.bak" -delete

# Rename files that contain "infdb-loader" or "infdb_loader"
find "$TARGET_DIR" -depth -name "*infdb-loader*" -o -name "*infdb_loader*" | while read -r file; do
    new_file=$(echo "$file" | sed "s/infdb-loader/$NEW_NAME/g" | sed "s/infdb_loader/$SNAKE_CASE_NAME/g")
    mv "$file" "$new_file"
done
