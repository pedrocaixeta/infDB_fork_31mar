...
CREATE TABLE {output_schema}.bld2grid AS
    ...
    JOIN {input_schema}.grid_cells g
        ON ST_Intersects(ST_transform(g.geom, 3035), (b.centroid));
...
FROM {input_schema}.buildings_lod2 bld
         CROSS JOIN LATERAL (
    SELECT bld.objectid, ts_metadata.id, ts_metadata.name, ST_transform(ts_metadata.geom, 3035) <-> bld.geom AS dist
...
SET geom = ST_ShortestLine(bld.centroid, ST_transform(ts.geom, 3035))
