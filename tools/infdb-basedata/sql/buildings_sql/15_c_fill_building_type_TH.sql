...
-- Step 3b: Buildings adjacent to at least 2 THs with similar floor area become TH
-- Note: this part is not transformed to graph-based solution yet, because the condition
-- SELECT DISTINCT n.a_id ...  HAVING COUNT(*) >= 2
-- means that a node is only labelled if two or more neighbors already have the label 'TH'.
DO
$$
    DECLARE
...
-- -- Note: this outcommented solution is only an approximate solution of 3b, labeling all buildings with two neighbours and at least one of them being 'TH'.
...
--       AND ABS(b1.floor_area - b2.floor_area) / GREATEST(b1.floor_area, b2.floor_area) < 0.25
-- ) b2 ON true;
--
...
--            COUNT(*) AS degree
--     FROM (
--         SELECT source AS node FROM building_edges
--         UNION ALL
--         SELECT target AS node FROM building_edges
--     ) t
--     GROUP BY node
-- )
-- SELECT c.component, c.id
-- FROM components c
-- JOIN node_degree d
--   ON c.id = d.node
-- WHERE d.degree >= 2; -- equivalent to HAVING COUNT(*) >= 2 in reference implementation
--
...
-- -- Note: the problem 3c can be solved with graph solution again.

-- Reference implementation with semi-procedural approach first.
-- -- Step 3c: Row of buildings with similar floor area and height likely TH
-- DO
-- $$
--     DECLARE
--         updated_count INTEGER := 1;
