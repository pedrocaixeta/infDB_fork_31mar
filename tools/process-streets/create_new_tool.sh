...
# Replace all occurrences of "process-streets" with the new name
echo "Replacing placeholders with $NEW_NAME..."
find "$TARGET_DIR" -type f -exec sed -i.bak "s/process-streets/$NEW_NAME/g" {} +
find "$TARGET_DIR" -type f -exec sed -i.bak "s/process_streets/$SNAKE_CASE_NAME/g" {} +
find "$TARGET_DIR" -type f -name "*.bak" -delete

# Rename files that contain "process-streets" or "process_streets"
find "$TARGET_DIR" -depth -name "*process-streets*" -o -name "*process_streets*" | while read -r file; do
    new_file=$(echo "$file" | sed "s/process-streets/$NEW_NAME/g" | sed "s/process_streets/$SNAKE_CASE_NAME/g")
    mv "$file" "$new_file"
done
