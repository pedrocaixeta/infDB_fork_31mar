...
CREATE TABLE {output_schema}.ways_heat_demand AS
    SELECT ways.way_id, ways.geom, sum(entise_output.demand_heating) as total_heating_demand, st_length(ways.geom) as length, sum(entise_output.demand_heating) / st_length(ways.geom) as demand_per_length
    ...
    GROUP BY ways.way_id, ways.geom;