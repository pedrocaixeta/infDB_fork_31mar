...
│   └── Functions
│
└── Schema: linear-heat-density         # Your new tool
   ├── Tables
   ├── Views
   └── Functions
```
...
Start tool
```bash
docker compose -f tools/linear-heat-density/compose.yml up
```

**Option B — VS Code Dev Containers:**
1. Open created tool folder in [*Visual Studio Code*](https://code.visualstudio.com/download): File → → Open Folder... → `tools/linear-heat-density`
...
```bash
# Remove docker
docker compose -f tools/linear-heat-density/compose.yml down
```
...
│   └── config-linear-heat-density.yml            # Tool-specific config
...
1. **Define tool name:**
   ...
   - Use kebab-case naming convention as for example "linear-heat-density" 

2. **Create Dev Container:**
   ```bash
   # Replace linear-heat-density
   bash tools/_infdb-template/create_new_tool.sh linear-heat-density
   ```

5. **Add dependencies:**
   - Add needed package into **dependencies** in `tools/linear-heat-density/pyproject.toml`.
   - Run `uv sync` in order to update virtual environment with new packages or (re-start) docker via `docker compose -f tools/linear-heat-density/compose.yml up`
